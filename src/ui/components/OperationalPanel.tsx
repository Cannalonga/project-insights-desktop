...
function getGapCompensationLabel(gapVsCompensation?: GapVsCompensation | null): string {
...
export function OperationalPanel({
  ...
  if (!score && !analysisReliability && !scheduleStatus && !gapVsCompensation && (!sCurve || sCurve.points.length === 0)) {
    return null;
  }
  ...
  return (
    <section className="dashboard-grid">
      <section className="decision-grid">
        ...
        <article className="panel-card executive-card">
          <span className="metric-label">Gap vs compensação</span>
          <strong>{getGapCompensationLabel(gapVsCompensation)}</strong>
          <span className="muted-text">
            {presentationMode === "executive" && primaryDecision
              ? `Ação líder ${primaryDecision.title} | ganho potencial ${formatPercent(primaryDecision.gainPercent)}`
              : gapVsCompensation?.gapPercent !== undefined
              ? `Gap ${formatPercent(gapVsCompensation.gapPercent)} | Top 3 ${formatPercent(gapVsCompensation.top3CompensationPercent)}`
              : gapVsCompensation?.message ?? "Sem base histórica suficiente"}
          </span>
        </article>
      </section>
        ...
        <section className="panel-card chart-card s-curve-primary-card">
          <div className="panel-header">
            <div>
              ...
              <h2 className="panel-title">Planejado x replanejado x real</h2>
            </div>
          ...
          <div className="chart-scroll">
            <svg
              width={getCurveChartWidth(sCurve.points.length)}
              height="320"
              viewBox={`0 0 ${getCurveChartWidth(sCurve.points.length)} 320`}
              role="img"
              aria-label="Curva S acumulada"
            >
              <line x1="56" y1="20" x2="56" y2="230" stroke="#94a3b8" strokeWidth="1" />
              <line x1="56" y1="230" x2={getCurveChartWidth(sCurve.points.length) - 24} y2="230" stroke="#94a3b8" strokeWidth="1" />
              {[0, 25, 50, 75, 100].map((value) => {
                const y = 230 - (value / 100) * 210;
                return (
                  <g key={value}>
                    <line x1="56" y1={y} x2={getCurveChartWidth(sCurve.points.length) - 24} y2={y} stroke="#e2e8f0" strokeWidth="1" />
          ...
          <div className="chart-legend minimal">
            ...
            <span><span className="legend-line" style={{ background: "#f97316" }} /> Replanejado</span>
